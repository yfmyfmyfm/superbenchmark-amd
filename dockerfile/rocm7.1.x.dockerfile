ARG BASE_IMAGE=rocm/pytorch:rocm7.1_ubuntu22.04_py3.10_pytorch_release_2.8.0
...
#   - hipblaslt: release-staging/rocm-rel-7.1
#   - rocblas: release-staging/rocm-rel-7.1
#   - openmpi: 4.1.9a1
# Intel:
#   - mlc: v3.12
...
# RUN export UBUNTU_VERSION=22.04
# RUN echo "Ubuntu version: $UBUNTU_VERSION"
ENV UBUNTU_VERSION=22.4
...
RUN if ! command -v ofed_info >/dev/null 2>&1; then \
    ...
    cd /tmp && \
    wget -q http://content.mellanox.com/ofed/MLNX_OFED-${OFED_VERSION}/MLNX_OFED_LINUX-${OFED_VERSION}-ubuntu22.04-x86_64.tgz && \
    tar xzf MLNX_OFED_LINUX-${OFED_VERSION}-ubuntu22.04-x86_64.tgz && \
    PATH=/usr/bin:${PATH} MLNX_OFED_LINUX-${OFED_VERSION}-ubuntu22.04-x86_64/mlnxofedinstall --user-space-only --without-fw-update --force --all && \
...
# Install RCCL
RUN cd /opt/ &&  \
    git clone -b release/rocm-rel-7.1 https://github.com/ROCmSoftwarePlatform/rccl.git && \
    cd rccl && \
    mkdir build && \
    cd build && \
...
ENV PATH="/usr/local/mpi/bin:/opt/superbench/bin:/usr/local/bin/:/opt/rocm/hip/bin/:/opt/rocm/bin/:${PATH}" \
    LD_PRELOAD="/opt/rccl/build/librccl.so" \
...
RUN make RCCL_HOME=/opt/rccl/build/ ROCBLAS_BRANCH=release/rocm-rel-7.1 HIPBLASLT_BRANCH=release/rocm-rel-7.1 ROCM_VER=release/rocm-rel-7.1 -C third_party rocm -o cpu_hpl -o cpu_stream -o megatron_lm
